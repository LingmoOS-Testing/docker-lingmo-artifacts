...
wget -O artifacts.tar.xz "https://github.com/LingmoOS-Testing/debuerreotype/releases/latest/download/artifacts.tar.xz"
tar -xvf artifacts.tar.xz
rm -v artifacts.tar.xz

# --strip-components 1
cp -rfv -f artifacts/amd64/* ./
rm -rfv artifacts
...
snapshotUrl="$(cat snapshot-url 2>/dev/null || echo 'https://packages.lingmo.org/')"
